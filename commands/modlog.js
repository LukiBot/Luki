...
exports.run = async (client, message, args, level) => {
	if (message.member.permissions.has("MANAGE_GUILD") == true) {
		if (!args[0]) return message.channel.send("Please specify the channel: `o!modlog #channelName/off`");
		if (message.mentions.channels == null || message.mentions.channels.first() == null) {
			if (args[0].toLowerCase() == "off") {
				db.get(`SELECT modlog FROM servers WHERE id=? `, [message.guild.id], (err, row) => {
					if (err) return console.log(err.message);
					if(!row) {
						db.run(`INSERT INTO servers(id) values(?)`, [message.guild.id], (err) => {
							console.log("added a server to servers database");
							db.run(`UPDATE servers SET modlog=? where id=?`, ["off", message.guild.id], (err) => {
								if (err) return console.log(err.message);
		...
		db.get(`SELECT modlog FROM servers WHERE id=? `, [message.guild.id], (err, row) => {
			...
			if(!row) {
				db.run(`INSERT INTO servers(id) values(?)`, [message.guild.id], (err) => {
					console.log("added a server to servers database");
					db.run(`UPDATE servers SET modlog=? where id=?`, [`${channelid}`, message.guild.id], (err) => {
						if (err) return console.log(err.message);
...
exports.help = {
  ...
  description: "Set the guild's moderation log",
  usage: "modlog #channelName/off"
};
